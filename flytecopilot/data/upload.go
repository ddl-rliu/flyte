...
func (u Uploader) RecursiveUpload(ctx context.Context, vars *core.VariableMap, uploadConfigs map[string]FileUploadConfig, errorFilePath string, metaOutputPath, dataRawPath storage.DataReference) error {
	childCtx, cancel := context.WithCancel(ctx)
	defer cancel()
